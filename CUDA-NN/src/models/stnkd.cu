...
void STNkd::name_params(std::map<std::string, Tensor*>& np) {
...
Tensor* STNkd::backward(Tensor* gradients) {
    size_t N = gradients->getSize(0), L = Configurer::cropping_size, C = 1024;
    gradients->reshape({N, k * k});
